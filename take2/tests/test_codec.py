...
from take2.types import Coord, PositionMessage, UnknownMessage


def test_should_encode_position_as_bytes() -> None:
    codec = Codec()
    coord = Coord(lat=63.0, lon=10.0, depth=5.0)
    result = codec.encode_position("001", coord)
...
def test_should_decode_position_bytes_to_position_message() -> None:
    codec = Codec()
    coord = Coord(lat=63.0, lon=10.0, depth=5.0)
    encoded = codec.encode_position("001", coord)
    ...
    assert abs(msg.coord.depth - 5.0) < 1e-3
...
def test_should_encode_sender_id_in_payload() -> None:
    ...
    encoded = codec.encode_position("042", coord)
    msg = codec.decode(encoded)
    assert isinstance(msg, PositionMessage)
...
def test_should_roundtrip_negative_coordinates() -> None:
    codec = Codec()
    coord = Coord(lat=-33.8688, lon=-151.2093, depth=12.5)
    encoded = codec.encode_position("007", coord)
    msg = codec.decode(encoded)
    assert isinstance(msg, PositionMessage)
    assert abs(msg.coord.lat - (-33.8688)) < 1e-4
    assert abs(msg.coord.lon - (-151.2093)) < 1e-4
    assert abs(msg.coord.depth - 12.5) < 1e-3
